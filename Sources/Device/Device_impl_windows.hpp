...
public:
	...
	bool setFormat(int width, int height, PixelFormat formatPix) {
		...
		if (FAILED(hvw::getFormat(_pDeviceConfig, width, height, formatPix == MJPG ? MEDIASUBTYPE_MJPG : MEDIASUBTYPE_YUY2, &_pFormat))) {
	...
	bool set(Device::Param code, double value) {
		...
		case Gamma:
			return hvw::setProp(VideoProcAmp_Gamma, value, _pPropFilter);
		}
	...
	double get(Device::Param code) {
		...
		case Gamma:
			return hvw::getProp(VideoProcAmp_Gamma, _pPropFilter);
		}
...
private:
	...
	bool _open(int idCamera) {
		...
		// Format 
		return get(Saturation) > 0 ? setFormat(1280, 720, MJPG) : setFormat(640, 480, MJPG);
	}
